# Collapse pass for Chapter 2 - wraps heavy sections in <details>
# Idempotent: detects already-wrapped sections and skips them.

$ErrorActionPreference = 'Stop'

$chapter2 = "d:\projects\whenthenonboarding\mastery-guide\02-api-development"

$book   = [char]::ConvertFromUtf32(0x1F4D6)   # book
$puzzle = [char]::ConvertFromUtf32(0x1F9E9)   # puzzle
$books  = [char]::ConvertFromUtf32(0x1F4DA)   # books
$emdash = [char]0x2014
...
$files = Get-ChildItem -Path $chapter2 -Filter "*.md" | Where-Object { $_.Name -ne 'README.md' }
...
foreach ($file in $files) {
    ...
    # Normalize Agent 1's existing drill wrap (it used no emoji prefix and ASCII hyphen).
    $normalized = $content -replace '<summary>Click to expand [—–\-] cross-question chains', "<summary>$book Click to expand $emdash cross-question chains"
    if ($normalized -ne $content) {
        $changes += "normalized-drill-summary"
        $content = $normalized
    }
    ...
    if ($content -ne $original) {
        ...
        $report += [pscustomobject]@{ File = $file.Name; Status = 'updated'; Changes = ($changes -join '; ') }
    } else {
        $report += [pscustomobject]@{ File = $file.Name; Status = 'no-change'; Changes = '' }
    }
}
